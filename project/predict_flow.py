...
def parse_args():
    ...
    elif args.corrupted_dataset_name in {'CIFAR-10-C-224x224', 'CIFAR-10-C', 'imagenet-c', 'imagenet100-c',
                                         'imagenet200-c'}:
...
def predict_with_noise():
    ...
    if args.results_file.exists():
        ...
        if args.corrupted_dataset_name in results_summary:
            ...
            results_summary[args.corrupted_dataset_name]['scores_versus_baseline'][
                args.baseline_model_logs_dir] = robustness_metrics
        else:
            results_summary[args.corrupted_dataset_name] = {
                'scores': scores,
                'scores_versus_baseline': {args.baseline_model_logs_dir: robustness_metrics}}
    else:
        results_summary = {args.corrupted_dataset_name: {
            'scores': scores,
            'scores_versus_baseline': {args.baseline_model_logs_dir: robustness_metrics}}}
