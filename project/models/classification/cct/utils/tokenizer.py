...
from project.models.utils.push_pull_unit import PushPullConv2DUnit
...
class Tokenizer(nn.Module):
    def __init__(self,
        ...
        i = 0
        if lightning_args.use_push_pull:
            _first_conv = PushPullConv2DUnit(n_filter_list[i], n_filter_list[i + 1],
                                            kernel_size=(kernel_size, kernel_size),
                                            avg_kernel_size=lightning_args.avg_kernel_size,
                                            pull_inhibition_strength=lightning_args.pull_inhibition_strength,
                                            trainable_pull_inhibition=lightning_args.trainable_pull_inhibition,
                                            stride=(stride, stride), padding=(padding, padding), bias=conv_bias)
        else:
            _first_conv = nn.Conv2d(n_filter_list[i], n_filter_list[i + 1],
                                    kernel_size=(kernel_size, kernel_size),
                                    stride=(stride, stride),
                                    padding=(padding, padding), bias=conv_bias)

        _first_layer = nn.Sequential(
            _first_conv,
        ...
        _remaining_layers = [nn.Sequential(
        ...
        self.conv_layers = nn.Sequential(_first_layer, *_remaining_layers)
