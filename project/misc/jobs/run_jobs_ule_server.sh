...
logs_dir="$HOME/runtime_data/pushpull-conv-long-training"
task="classification"
...
common_train_args="--accelerator gpu --img_size 224 --model ${model} --hash_length 64 --quantization_weight 1e-4 --num_workers ${num_workers} --batch_size 64 --max_epochs 50 --weight_decay 5e-5 --lr_base 5e-2 --task ${task} --logs_dir ${logs_dir} --experiment_name ${experiment_name} --dataset_dir ${dataset_dir} --dataset_name ${dataset_name}"
...
do
    ...
    # Evaluation Phase
        ...
        if ((avg == -1)); then
            python ${predict_script} --no-use_push_pull --models_to_predict all --predict_model_logs_dir "${experiment_dir}" ${common_predict_args} --devices "$((i%2))," > "${experiment_dir}/logs_predict.out" 2>&1
        else
            python ${predict_script} --models_to_predict all --predict_model_logs_dir "${experiment_dir}" ${common_predict_args} --baseline_model_logs_dir ${baseline_model_logs_dir} --devices "$((i%2))," > "${experiment_dir}/logs_predict.out" 2>&1
        fi
    fi
